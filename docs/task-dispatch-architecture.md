...
业务层：帝国制度 (Imperial Governance Model)
  ├─ 分权制衡：主人 → 公主 → 中书 → 门下 → 尚书 → 六部
  ├─ 制度约束：不可越级、状态严格递进、门下必审议
  └─ 质量保障：可封驳反工、实时可观测、紧急可干预
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
技术层：OpenClaw多Agent编排 (Multi-Agent Orchestration)
  ├─ 状态机：9个状态（Pending → Gongzhu → Zhongshu → Menxia → Assigned → Doing/Next → Review → Done/Cancelled）
...
stateDiagram-v2
[*] --> Pending: 主人下旨
Pending --> Gongzhu: 公主接旨
Gongzhu --> Zhongshu: 公主转交中书
...
DAY 1:
  10:00 - 主人飞书："为三省六部编写完整自动化测试方案"
          公主接旨。state = Gongzhu, org = 公主
          自动派发 gongzhu agent → 处理此旨意
  
  10:30 - 公主分拣完毕。判定为「工作旨意」（非闲聊）
          建任务 JJC-20260228-E2E
          flow_log 记录："主人 → 公主：下旨"
          state: Gongzhu → Zhongshu, org: 公主 → 中书省
          自动派发 zhongshu agent
...
| 契约 | 含义 | 违反后果 |
|------|------|---------|
| **不可越级** | 公主只能调中书，中书只能调门下/尚书，六部不能对外调用 | 超权调用被拒绝，系统自动拦截 |
| **状态单向递进** | Pending → Gongzhu → Zhongshu → ... → Done，不能跳过或倒退 | 只能通过 review_action(reject) 返回上一步 |
...
kind    count  代表事件
────────────────────────────────────────────────
flow      10   状态转移链（Pending→Gongzhu→Zhongshu→...）
...
FOR EACH 任务:
  ...
  IF retryCount < maxRetry:
    ...
    - dispatch_for_state(task, new_state, trigger='gongzhu-scan-retry')
...
T+180:
  ...
  ✅ 阶段1：重试
  - retryCount: 0 → 1
  - dispatch_for_state('JJC-20260228-E2E', 'Zhongshu', trigger='gongzhu-scan-retry')
...
T+720 (若仍未解决):
  ...
  - dispatch_for_state('JJC-20260228-E2E', 'Assigned', trigger='gongzhu-auto-rollback')
...
响应：
{
  ...
  "message": "旨意 JJC-20260228-001 已下达，正在派发给公主"
}
```
...
响应：
{
  ...
  "relatedAgents": ["gongzhu", "zhongshu", "menxia"],
  "phaseDurations": [
    {
      "phase": "公主",
      "durationText": "30分",
...
# 说明：
# - 第一个参数：task_id
# - 第二个参数：新状态（Pending/Gongzhu/Zhongshu/...）
...
# 门下省想升级到主人
try:
    result = dispatch_to_agent('gongzhu', '我需要主人的指示')
except PermissionError:
    # ❌ 权限矩阵拦截
    log.error('menxia 无权调用 gongzhu')
```
