...
const STATE_ORDER: Record<string, number> = {
  ...
  Huanghou: 5, Inbox: 6, Blocked: 7, Next: 8, Done: 9, Cancelled: 10,
};
...
export default function EdictBoard() {
  ...
  const handleScan = async () => {
    try {
      ...
      if (r.ok) toast(`🧭 皇后巡检完成：${r.count || 0} 个动作`);
  ...
  return (
    <div>
      ...
      <div className="archive-bar">
        ...
        <button className="ab-scan" onClick={handleScan}>🧭 皇后巡检</button>
      </div>
