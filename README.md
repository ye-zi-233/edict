...
<p align="center">
  <sub>12 个 AI Agent（11 个业务角色 + 1 个兼容角色）组成三省六部：公主分拣、中书省规划、门下省审核封驳、尚书省派发、六部+吏部并行执行。<br>比 CrewAI 多一层<b>制度性审核</b>，比 AutoGen 多一个<b>实时看板</b>。</sub>
</p>
...
```
你 (主人) → 公主 (分拣) → 中书省 (规划) → 门下省 (审议) → 尚书省 (派发) → 六部 (执行) → 回奏
```
...
- **🌸 女娲（meta-agent）** —— 只读分析各 Agent 灵魂与对话，起草「灵魂调优」提案，提交主人审批；发现能力缺口时向主人提出建议。**不直接修改**任何配置，批准后由主人执行 `apply_nvwa_proposal.py` 落地。

### 🌸 女娲与提案审批

女娲不参与日常旨意流转，只做两件事：**感应灵魂**（只读 SOUL.md 与对话日志）和**起草调优提案**（修改某 Agent 人格）。所有变更均以**提案 → 主人审批 → 脚本执行**的方式完成。发现能力缺口时，女娲会向主人提出建议，由主人决定是否新增 Agent。

| 角色 | 脚本 | 用途 |
|------|------|------|
| 女娲 | `scripts/nvwa_tools.py` | `list-agents` / `read-soul` / `read-logs` / `propose modify-soul` |
| 主人 | `scripts/apply_nvwa_proposal.py` | `list` / `show <id>` / `approve <id>` / `reject <id>` |

提案存放在 `data/nvwa_proposals/`（或女娲 workspace 内对应目录），批准后 `approve` 会写入 `agents/<id>/SOUL.md`，sync worker 下次同步时自动部署到 workspace。
...
打开 http://localhost:3000 即可使用军机处看板（全功能）。

> 详细部署文档见 [edict/README.md](edict/README.md)
...
| `FRONTEND_PORT` | 看板对外端口 | `3000` |
| `BACKEND_PORT` | API 对外端口 | `8000` |
| `SYNC_INTERVAL` | 数据刷新间隔（秒） | `15` |

> NAS 用户请将 `PUID`/`PGID` 设为与 OpenClaw 目录相同的 UID/GID（可通过 `id` 命令或 NAS 管理界面查看）。
...
| 部门 | Agent ID | 职责 | 擅长领域 |
|------|----------|------|---------|
| 👑 **公主** | `gongzhu` | 消息分拣、需求整理 | 闲聊识别、旨意提炼、标题概括 |
...
| 🔮 **女娲** | `nvwa` | 灵魂守护、Agent 调优 | SOUL.md 分析、引导词优化、能力缺口建议 |

### 权限矩阵

> 不是想发就能发 —— 真正的分权制衡

| From ↓ \ To → | 公主 | 中书 | 门下 | 尚书 | 户 | 礼 | 兵 | 刑 | 工 | 吏 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **公主** | — | ✅ | | | | | | | | |
| **中书省** | ✅ | — | ✅ | ✅ | | | | | | |
| **门下省** | | ✅ | — | ✅ | | | | | | |
...
```
主人 → 公主分拣 → 中书规划 → 门下审议 → 已派发 → 执行中 → 待审查 → ✅ 已完成
                      ↑          │                              │
                      └──── 封驳 ─┘                    阻塞 Blocked
```
...
│   ├── gongzhu/SOUL.md        # 公主 · 消息分拣（含旨意标题规范）
...
| **Docker 部署** | `docker compose up -d` 一键启动全部服务 |
...
<details>
<summary><b>❌ 任务总超时 / 下属完成了但无法传回公主</b></summary>

**症状**：六部或尚书省已完成任务，但公主收不到回报，最终超时。
...
```
确认 `gongzhu` agent 的 `statusLabel` 是 `alive`。
...
3. **常见原因**：
   - Agent ID 不匹配（已在 v1.3 修复：`taizi` → `gongzhu`）
...
### Phase 1 — 核心架构 ✅
- [x] 十二部制 Agent 架构（公主 + 三省 + 七部 + 早朝官）+ 权限矩阵
