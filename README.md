...
<p align="center">
  <sub>12 个 AI Agent（11 个业务角色 + 1 个兼容角色）组成三省六部：皇后分拣、中书省规划、门下省审核封驳、尚书省派发、六部+吏部并行执行。<br>比 CrewAI 多一层<b>制度性审核</b>，比 AutoGen 多一个<b>实时看板</b>。</sub>
</p>
...
```
你 (主人) → 皇后 (分拣) → 中书省 (规划) → 门下省 (审议) → 尚书省 (派发) → 六部 (执行) → 回奏
```
...
- **🌸 女娲（meta-agent）** —— 只读分析各 Agent 灵魂与对话，起草「修改灵魂」或「新建 Agent」的提案，提交主人审批；**不直接修改**任何配置，批准后由主人执行 `apply_nvwa_proposal.py` 落地。

### 🌸 女娲与提案审批

女娲不参与日常旨意流转，只做两件事：**感应灵魂**（只读 SOUL.md 与对话日志）和**起草提案**（修改某 Agent 人格 / 新建 Agent）。所有变更均以**提案 → 主人审批 → 脚本执行**的方式完成。

| 角色 | 脚本 | 用途 |
|------|------|------|
| 女娲 | `scripts/nvwa_tools.py` | `list-agents` / `read-soul` / `read-logs` / `propose modify-soul` / `propose create-agent` |
| 主人 | `scripts/apply_nvwa_proposal.py` | `list` / `show <id>` / `approve <id>` / `reject <id>` |

提案存放在 `data/nvwa_proposals/`（或女娲 workspace 内对应目录），批准后 `approve` 会写入 `agents/<id>/SOUL.md`；新建 Agent 需再在 `sync_agent_config.py` 与 `install.sh` 中注册。
...
- Gateway 已启用 HTTP API（在 Gateway 配置中设置 `gateway.http.endpoints.responses.enabled = true`）
...
打开 http://localhost:7891 即可使用军机处看板（全功能）。
...
| 部门 | Agent ID | 职责 | 擅长领域 |
|------|----------|------|---------|
| 👑 **皇后** | `huanghou` | 消息分拣、需求整理 | 闲聊识别、旨意提炼、标题概括 |
...
| 🔮 **女娲** | `nvwa` | 灵魂守护、Agent 调优 | SOUL.md 管理、引导词优化、新 Agent 创建 |

### 权限矩阵

> 不是想发就能发 —— 真正的分权制衡

| From ↓ \ To → | 皇后 | 中书 | 门下 | 尚书 | 户 | 礼 | 兵 | 刑 | 工 | 吏 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **皇后** | — | ✅ | | | | | | | | |
| **中书省** | ✅ | — | ✅ | ✅ | | | | | | |
| **门下省** | | ✅ | — | ✅ | | | | | | |
...
```
主人 → 皇后分拣 → 中书规划 → 门下审议 → 已派发 → 执行中 → 待审查 → ✅ 已完成
                      ↑          │                              │
                      └──── 封驳 ─┘                    阻塞 Blocked
```
...
│   ├── huanghou/SOUL.md        # 皇后 · 消息分拣（含旨意标题规范）
...
├── Dockerfile                  # Docker 镜像构建（dashboard + sync 共用）
├── docker-compose.yml          # Docker Compose 全功能部署编排
├── .env.example                # Docker 部署配置模板
├── install.sh                  # 非 Docker 安装脚本（可选）
...
### Phase 1 — 核心架构 ✅
- [x] 十二部制 Agent 架构（皇后 + 三省 + 七部 + 早朝官）+ 权限矩阵
